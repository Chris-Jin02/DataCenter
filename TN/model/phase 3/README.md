# Phase 3: input and spatial-validation registry

Phase 3A through 3G are complete. The frozen Phase 2 workbook remains unchanged (SHA-256: `112d6db67d7f4f9e74fa8edde3406937e1f8eb7145e9bc04a64e7030cf1d3e03`); its training inputs, field contract, and spatial partitions are the registered interface for subsequent model training.
...
| [Statewide supervised ranking](results/phase3f_supervised_statewide_predictions.xlsx) | 60 full-D1 `screened_logistic` fits, 0–100 Primary relative scores, draw uncertainty, scenario rank ranges, coefficients, and site mappings |
...
Phase 3F is complete. Under the registered 3D rule, `screened_logistic` is fixed as the transparent supervised specification. It uses `dist_major_road_km`, `dist_transmission_any_km`, and `dist_surface_water_flowline_km`, fitted for each of 20 10:1 draws in the Primary, Weighted, and Strict scenarios, for 60 full-D1 training runs. All converged; overlap between background and occupied cells, and non-D1 background rows, are both zero. The Primary relative empirical score is the 0–100 within-D1 percentile rank of the median linear prediction across 20 Primary draws. The workbook also retains draw IQR, scenario rank ranges, and cell-score mappings for 74 Presence sites. A registered Phase 3B west/draw-1 holdout run reproduced with zero error.

Phase 3G is complete. All 4,391 D1 cells are joined with 3E regimes, stability, Weighted Presence enrichment, and novelty, as well as 3F Primary scores, draw uncertainty, and three-scenario rank ranges. The Primary supervised score remains the only numeric ranking; unsupervised results provide corroboration, exceptions, and review states only. R2 accounts for 25.3% of D1 but 50.9% of Primary ≥90 cells and 43.7% of Primary ≥75 cells. The next step is 3H: freeze Phase 3 and make the scoring-release decision.
